...
public class RunPayrollViewModel {
	...
	RunPayRollBean exPdfSheetBean = new RunPayRollBean();
	...
	private SheetBean exSheetTypeBean = new SheetBean();
	...
	private ArrayList<SheetBean> exSheetTypeBeanList = new ArrayList<SheetBean>();
	//private ArrayList<E>
	...
	ArrayList<RunPayRollBean> exPdfSheetBeanList = new ArrayList<RunPayRollBean>();
	...
	public void initSetUp(@ContextParam(ContextType.VIEW) Component view) throws Exception{
		...
		exSheetTypeBeanList = RunPayRollDao.loadExSheetTypeList();
		loadLeaveYrDate();
	...
	public void onSelectMonth(){
		...
		unitDesignationBeanList.clear();
		sheetTypeBeanList.clear();
	...
	public void onSelectType(){
		...
		runPayRollBean.setSheetTypeId(sheetbean.getSheetTypeId());
			...
			for(RunPayRollBean bean : runPayRollBeanList){
				...
				if(bean.getPresentDay()!=null ){
					...
					//if(bean.getEmpDesignation().equalsIgnoreCase("EX-SERVICE MAN") || bean.getEmpDesignation().equalsIgnoreCase("EX-SERVICE MAN SUPERVISOR") || bean.getEmpDesignation().equalsIgnoreCase("GUN MAN")){
						...
						for(EmployeeSalaryComponentAmountBean earn: earningList){
							...
							if(companyMasterBean.getCompanyId()==48){
								if(bean.getEmpDesignation().equalsIgnoreCase("CARE TAKER") && earn.getComponentName().equalsIgnoreCase("ALLOWANCE")){
									...
									earn.setComponentAmount(Rules.getGeneral(earn.getComponentAmount(), bean.getBaseDays(), bean.getPresentDay()));
									System.out.println("FOR RLG COURT 4>>> >> > " + earn.getComponentAmount() + " - " + earn.getComponentName());
								}
							}
						...
						pdfBean.setSheetTypeId(sheetbean.getSheetTypeId());
						...
						pdfBean.setOverTime(0.0);
						pdfBean.setOverTimeSal(0.0);
					...
					for(EmployeeSalaryComponentAmountBean earn: earningList){
						...
						if(bean.getOverTime()==null && earn.getComponentName().equalsIgnoreCase("ALLOWANCE") 
							...
							if(bean.getSelectedUnitId()==29){
								...
								System.out.println("alal=---------------------=============------------------ >>> >> > " + earn.getComponentAmount());
							}
						...
						/*if(earn.getComponentName().equalsIgnoreCase("ALLOWANCE") && bean.getSelectedUnitId()==29){
							double allowance = Rules.getAllowances(earn.getComponentAmount(), bean.getBaseDays(), bean.getPresentDay());
							earn.setComponentAmount(allowance);
						}*/
	...
	public void onclickSaveSalSheet(){
		...
		if(isChecked){
			...
			RunPayRollService.saveEmpSalStore(selectedEmployeeList, pdfSheetBean, companyMasterBean.getCompanyId(), unitMasterBean.getUnitId(), unitDesignationBean.getUnitDesignationId(), monthMasterBean.getMonthName(), String.valueOf(year), sheetbean.getSheetTypeId());
		}else{
			Messagebox.show("Please check at least one!","Alert Information",Messagebox.OK,Messagebox.EXCLAMATION);
		}
	...
	public void onSelectYr2(){
		if(payrollExistBeanList.size()>0){
			payrollExistBeanList.clear();
		}
		exPdfSheetBeanList.clear();
		companyMasterBean2.setCompanyName(null);
		unitMasterBean2.setUnitName(null);
		unitDesignationBean2.setUnitDesignation(null);
		exSheetTypeBean.setSheetType(null);
		monthBean2.setMonthName(null);
		
		monthList2.clear();
		companyBeanList2.clear();
		unitMasterBeanList2.clear();
		unitDesignationList2.clear();
		exSheetTypeBeanList.clear();
	...
	public void onClickLoad2(){
		...
		exPdfSheetBeanList.clear();
		payrollExistBeanList = RunPayRollService.loadEmpSalStore(exPayrollBn, exSheetTypeBean.getSheetTypeId());
		
		for(RunPayRollBean bean : payrollExistBeanList){
			...
			if(bean.getPresentDay()!=null ){
				...
				pdfBean.setSheetType(exSheetTypeBean.getSheetType());
				pdfBean.setSheetTypeId(exSheetTypeBean.getSheetTypeId());
				//System.out.println("ex PDF TYPE " + exSheetTypeBean.getSheetType());
				pdfBean.setPresentDay(bean.getPresentDay());
				pdfBean.setWages(bean.getWages());
				pdfBean.setBasic(bean.getBasic());
				...
				pdfBean.setEmpEsi(bean.getEmpEsi());
				pdfBean.setEmpUan(bean.getEmpUan());
				pdfBean.setHoliDayAmount(bean.getHoliDayAmount());
				pdfBean.setOverTime(bean.getOverTime());
				
				pdfBean.setOverTimeSal(bean.getOverTimeSal());
				
				pdfBean.setEarningCompList(earningList);
				pdfBean.setDeductionCompList(deductionList);
				exPdfSheetBeanList.add(pdfBean);
	...
	public void onClickRemove(){
		if(exPayrollBn.getCompanyId2()>0 && exPayrollBn.getUnitId2()>0 && exPayrollBn.getUnitDesignationId2()>0 && exPayrollBn.getSalMonth2() != null && exPayrollBn.getLvYr2() !=null && payrollExistBeanList.size()>0){
	...
	@Command
	@NotifyChange("*")
	public void onSelectExSheetType(){
		System.out.println("sh ty -- -- -->>> >> > " + exSheetTypeBean.getSheetType());
		System.out.println("sh id -- -- -->>> >> > " + exSheetTypeBean.getSheetTypeId());
		
	}
	
	@Command
	@NotifyChange("*")
	public void onClickExSheetGenerate(){

		String exPdfPath = Executions.getCurrent().getDesktop().getWebApp().getRealPath("/");
		exPdfSheetBean.setComapnyName(companyMasterBean2.getCompanyName());
		exPdfSheetBean.setUnitName(unitMasterBean2.getUnitName());
		exPdfSheetBean.setCurrentDate(currentDate);
		exPdfSheetBean.setMonthName(monthBean2.getMonthName());
		exPdfSheetBean.setYear(String.valueOf(lvYrList2));
		exPdfSheetBean.setUnitDesignation(unitDesignationBean2.getUnitDesignation());
		exPdfSheetBean.setSheetType(exSheetTypeBean.getSheetType());
		ExistingPaymentDetailsViewModel paySlipGenerator = new ExistingPaymentDetailsViewModel();
		ArrayList<RunPayRollBean> selectedEmployeeList = new ArrayList<RunPayRollBean>();
	    boolean isChecked =  false;
	    
		if(exPdfSheetBeanList.size()>0){
			//System.out.println("* * * * * * * On Click generate sheet * * * *");
			 for(RunPayRollBean bean : selectedEmployeeList){
				 for(EmployeeSalaryComponentAmountBean comp : bean.getEarningCompList()){
					 //System.out.println(comp.toString());
			...
			try {
				paySlipGenerator.getSheetDetails(exPdfPath, exPdfSheetBeanList, exPdfSheetBean);
			...
			Messagebox.show("Data not available!","Alert Information",Messagebox.OK,Messagebox.EXCLAMATION);
		}
	...
	@Command
	@NotifyChange("*")
	public void onClickExSlipGenerate(){
		
		ArrayList<RunPayRollBean> selectedExEmployeeList = new ArrayList<RunPayRollBean>();
		
		System.out.println("expdfbeanlist size " + exPdfSheetBeanList.size());
		
		for(RunPayRollBean payRollBean : exPdfSheetBeanList){
			System.out.println("Id - selected >>> >> > " + exPdfSheetBeanList.indexOf(payRollBean) +" - " + payRollBean.isExChecked());
	    	if(payRollBean.isExChecked()){
	    		selectedExEmployeeList.add(payRollBean);
	    		
	    	}
	    }
		
		
		if(selectedExEmployeeList.size()>0 && selectedExEmployeeList.size()<6){
			String exPdfPath = Executions.getCurrent().getDesktop().getWebApp().getRealPath("/");
			ExistingPaymentDetailsViewModel paySlipGenerator = new ExistingPaymentDetailsViewModel();
			...
			try {
				paySlipGenerator.getSlipDetails(exPdfPath, selectedExEmployeeList, exPdfSheetBean);
			} catch (DocumentException e) {
			...
			Messagebox.show("Please check at least one \n or maximum five","Alert Information",Messagebox.OK,Messagebox.EXCLAMATION);
		}
	...
	public ArrayList<SheetBean> getExSheetTypeBeanList() {
		return exSheetTypeBeanList;
	}

	public void setExSheetTypeBeanList(ArrayList<SheetBean> exSheetTypeBeanList) {
		this.exSheetTypeBeanList = exSheetTypeBeanList;
	}

	public SheetBean getExSheetTypeBean() {
		return exSheetTypeBean;
	}

	public void setExSheetTypeBean(SheetBean exSheetTypeBean) {
		this.exSheetTypeBean = exSheetTypeBean;
	}
	...
	public RunPayRollBean getExPdfSheetBean() {
		return exPdfSheetBean;
	}

	public void setExPdfSheetBean(RunPayRollBean exPdfSheetBean) {
		this.exPdfSheetBean = exPdfSheetBean;
	}

	public ArrayList<RunPayRollBean> getExPdfSheetBeanList() {
		return exPdfSheetBeanList;
	}

	public void setExPdfSheetBeanList(ArrayList<RunPayRollBean> exPdfSheetBeanList) {
		this.exPdfSheetBeanList = exPdfSheetBeanList;
	}
