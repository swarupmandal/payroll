...
public class PayRollExistDao {
	...
	public static ArrayList<RunPayRollBean> loadEmpSalDetails2(PayrollExistBean pxBean, int exSheetTypeId){
		...
		try {
			...
			try {
				preparedStatement = Util1.createQuery(connection, RunPayRollSql.empSalStoreSelectQuery, Arrays.asList(pxBean.getCompanyId2(), 
						pxBean.getUnitId2(), pxBean.getUnitDesignationId2(), pxBean.getSalMonth2(), pxBean.getLvYr2(), exSheetTypeId));
				...
				while (resultSet.next()) {
					...
					bean.setOverTime(resultSet.getDouble("extra_time"));
					bean.setOverTimeSal(resultSet.getDouble("extra_amt"));
					bean.setComponentAmountBeanList(loadCompoAmtDtls(connection, bean.getEmpId(),pxBean.getSalMonth2(), pxBean.getLvYr2(), exSheetTypeId));
	...
	public static ArrayList<EmployeeSalaryComponentAmountBean> loadCompoAmtDtls(Connection connection, int empId, String month, String year, int exsheettypeid){
		...
		try {
			...
			preparedStatement = Util1.createQuery(connection, RunPayRollSql.empSalStoreCompoDetlQuery, 
					Arrays.asList(empId , month, year, exsheettypeid));
